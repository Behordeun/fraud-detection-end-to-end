...
def create_model_performance_chart(metrics):
    ...
    fig.add_trace(
        go.Indicator(
            ...
            title={"text": "F1 Score"},
            gauge={"axis": {"range": [None, 1]}, "bar": {"color": "darkred"}},
        ),
        row=2,
        col=2,
    )
...
def main():
    ...
    with col1:
        st.plotly_chart(
            create_model_performance_chart(metrics), use_container_width=True
        )

    with col2:
        st.plotly_chart(
            create_prediction_distribution(predictions_df), use_container_width=True
        )

    # Fraud probability distribution
    st.plotly_chart(
        create_fraud_probability_dist(predictions_df), use_container_width=True
    )
