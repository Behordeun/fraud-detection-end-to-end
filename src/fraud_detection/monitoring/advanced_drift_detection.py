...
class AdvancedDriftDetector:
    ...
    def detect_feature_drift(self, new_data):
        ...
        for column in self.reference_data.columns:
            if column in new_data.columns and pd.api.types.is_numeric_dtype(
                ...
                ks_stat, ks_p_value = stats.ks_2samp(
                    self.reference_data[column], new_data[column]
                )
    ...
    def generate_drift_report(self, new_data_path, output_path="drift_report.json"):
        ...
        print(
            f"Drift Report: {drift_summary['drift_percentage']:.1f}% of features show drift"
        )
