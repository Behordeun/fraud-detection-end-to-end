    ...
    def __init__(self):
        ...
        self.models = {
            "RandomForest": RandomForestClassifier(
                featuresCol="features", labelCol="Class", numTrees=100
            ),
            "GradientBoosting": GBTClassifier(
                featuresCol="features", labelCol="Class", maxIter=20
            ),
            "LogisticRegression": LogisticRegression(
                featuresCol="features", labelCol="Class", maxIter=100
            ),
        }
        self.results = []
    ...
    def evaluate_model(self, name, model, train_data, test_data):
        ...
        with mlflow.start_run(run_name=f"model_comparison_{name}"):
            ...
            recall = multi_eval.evaluate(
                predictions, {multi_eval.metricName: "weightedRecall"}
            )
